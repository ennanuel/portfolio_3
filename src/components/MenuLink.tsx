...
export default function MenuLink({ platform, url_link, target, className }: { platform: string, target?: string; url_link: string; className?: string; }) {


    return (
        <a href={url_link} target={target} className="group w-max relative overflow-hidden -ml-2 block">
            <span className={`${className} text-sm sm:text-base block relative z-[1] pr-4 md:pr-6 pl-2 tracking-tight`}>{platform}</span>
