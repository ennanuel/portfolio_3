...
export default function Button({ text, Icon, size, className, hoverTextClassName, onClick, toLeft, noGap, namespace }: Props) {
    ...
    return (
        <button 
            ...
            data-cal-link={namespace ? "ennanuel/30min": null}
            data-cal-config={namespace ? '{"layout":"month_view"}' : null}
            className={`${className ?  className : 'font-ov-soge bg-brown-800 text-brown-300 rounded-full sm:rounded-lg min-h-12 md:min-h-[60px] px-6 md:px-8 py-2 md:py-4'} text-sm sm:text-base group overflow-hidden relative flex items-center justify-center before:absolute before:top-1/2 before:-translate-y-1/2 before:w-full before:h-full before:rounded-full before:bg-brown-600/50 before:transition-[transform,_border-radius] ${toLeft ? 'before:right-0 before:translate-x-[101%]' : 'before:left-0 before:-translate-x-[101%]'} hover:before:rounded-none before:duration-500 before:ease-expo hover:before:-translate-x-1/2 hover:before:scale-[2]`}
        >
